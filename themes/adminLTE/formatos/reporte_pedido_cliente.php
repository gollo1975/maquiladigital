...
class PDF extends FPDF {
    ...
    function Body($pdf,$model) {
        ...
        $detalles = PedidoClienteReferencias::find()->where(['=','id_pedido',$model->id_pedido])->all();
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 8);
        $cant = 0;
        foreach ($detalles as $detalle) { 
            ...
            $pdf->Cell(23, 4, number_format($detalle->iva, 0, '.', ','), 0, 0, 'R');
        ...
        $pdf->SetXY(177, 232);
        $pdf->MultiCell(25, 8, number_format($model->total_pedido, 0, '.', ','),1,'R',1);
