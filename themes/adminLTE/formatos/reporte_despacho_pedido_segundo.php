...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $despacho = DespachoPedidos::findOne($id_despacho);
        ...
        $this->Cell(162, 7, utf8_decode("DESPACHO DEL PEDIDO (TALLAS Y COLORES)"), 0, 0, 'l', 0);
        ...
        $this->Cell(75, 5, utf8_decode($despacho->cliente->cedulanit.'-'.$despacho->cliente->dv), 0, 0, 'L',1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("Cliente:"), 0, 0, 'c', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(71, 5, utf8_decode($despacho->cliente->nombrecorto), 0, 0, 'c', 1);
        
        $this->SetXY(10, 53);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(26, 5, utf8_decode("Departamento:"), 0, 0, 'l', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(75, 5, utf8_decode($despacho->cliente->departamento->departamento), 0, 0, 'L',1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("Municipio:"), 0, 0, 'l', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(71, 5, utf8_decode($despacho->cliente->municipio->municipio), 0, 0, 'L', 1);
        
        $this->SetXY(10, 57);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(26, 5, utf8_decode("Fecha despacho:"), 0, 0, 'l', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(75, 5, utf8_decode($despacho->fecha_despacho), 0, 0, 'L',1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("Cantidad:"), 0, 0, 'l', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(71, 5, utf8_decode($despacho->cantidad_despachada), 0, 0, 'L', 1);
        
        $this->SetXY(10, 61);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(26, 5, utf8_decode("Hora registro:"), 0, 0, 'L', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(75, 5, utf8_decode($despacho->fecha_hora_registro), 0, 0, 'l', 1);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("No pedido:"), 0, 0, 'J', 1);
        $this->SetFont('Arial', '', 7);
        $this->Cell(71, 5, utf8_decode($despacho->numero_pedido), 0, 0, 'L', 1);
    ...
    function Body($pdf, $model) {
        $detalles = \app\models\DespachoPedidoDetalles::find()->where(['=','id_despacho',$model->id_despacho])->all();
        ...
        foreach ($detalles as $detalle) {
            ...
            ])->andWhere(['>','segundo_despacho', 0])->all();
            if(count($tallas) === 0){
                    $tallas = PedidoTallas::find()->where([
                    'id_pedido' => $model->id_pedido,
                    'id_detalle' => $detalle->id_detalle,
                        ])->andWhere(['>','unidades_despachadas', 0])->all();
            }
            ...
            foreach ($tallas as $talla){
                ...
                if (empty($colores)) {
                    ...
                    if ($talla->segundo_despacho == 0){
                        $groupedItems[$key]['unidades'] += $talla->unidades_despachadas;
                    }else{
                       $groupedItems[$key]['unidades'] += $talla->segundo_despacho; 
                    }
                    ...
                    foreach ($colores as $color) {
                        ...
                        $groupedItems[$key]['unidades'] += $color->cantidad;
                    }
                }
            }
        }
...
$pdf->Output("Tallas_Pedido_$model->numero_despacho.pdf", 'D');
