...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(19, 5, utf8_decode("Municipio:"), 0, 0, 'l', 1);
         $this->SetFont('Arial', '', 8);
        $this->Cell(61, 5, utf8_decode($config->municipio->municipio." - ".$config->departamento->departamento), 0, 0, 'L', 1);
        $this->SetXY(60, 5);
        //FIN
        $this->SetXY(10, 32);
        ...
        $this->Line(30,68,30,200);
        $this->Line(107,68,107,200);
        $this->Line(130,68,130,200);
        $this->Line(153,68,153,200);
        $this->Line(176,68,176,200);
    ...
    function EncabezadoDetalles() {
        $this->Ln(7);
        $header = array('CODIGO', 'DESCRIPCION REFERENCIA', 'CANTIDAD', 'VLR UNITARIO','% DESCTO','SUBTOTAL');
        $this->SetFillColor(200, 200, 200);
        $this->SetTextColor(0);
    ...
    function Body($pdf,$model) {
        ...
        foreach ($detalles as $detalle) { 
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(20, 4, $detalle->inventario->codigo_producto, 0, 0, 'L');
            $pdf->Cell(77, 4, utf8_decode($detalle->inventario->nombre_producto) , 0, 0, 'L');
            $pdf->Cell(23, 4, $detalle->cantidad, 0, 0, 'R');
            $pdf->Cell(23, 4, number_format($detalle->valor_unitario, 0, '.', ','), 0, 0, 'R');
            $pdf->Cell(23, 4, number_format($detalle->porcentaje_descuento, 0, '.', ','), 0, 0, 'R');
            $pdf->Cell(26, 4, number_format($detalle->total_linea, 0, '.', ','), 0, 0, 'R');
        ...
        $pdf->SetXY(10, 200);
        $this->SetFont('Arial', 'B', 8);
        $pdf->MultiCell(146, 4, utf8_decode(valorEnLetras($model->total_pedido)),0,'J');
        $pdf->SetXY(156, 200);
        $pdf->MultiCell(20, 8, 'SUBTOTAL:',1,'C');
        $pdf->SetXY(176, 200);
        $pdf->MultiCell(26, 8, number_format($model->valor_total, 0, '.', ','),1,'R');
        //
        $pdf->SetXY(10, 208);
        $this->SetFont('Arial', 'B', 8);
        ...
        $pdf->SetXY(176, 208);
        $pdf->MultiCell(26, 8, number_format(0, 0, '.', ','),1,'R');
        //
        $pdf->SetXY(156, 216);
        $pdf->MultiCell(20, 8, 'IVA:',1,'C');
        $pdf->SetXY(176, 216);
        $pdf->MultiCell(26, 8, number_format($model->impuesto, 0, '.', ','),1,'R');
        //
        $pdf->SetXY(156, 224);
...
//
        ...
        $pdf->SetXY(176, 232);
        $pdf->MultiCell(26, 8, number_format($model->total_pedido, 0, '.', ','),1,'R',1);
