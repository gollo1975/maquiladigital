...
<div class="orden-produccion-insumos-view">
        ...
        <?php }else{ 
            ...
            <?php }else{ ?>
                ...
                <?= Html::a('<span class="glyphicon glyphicon-send"></span>  Descargar insumos', ['enviar_insumos_modulo', 'id' => $model->id_entrega],['class' => 'btn btn-info btn-xs',
                        'data' => ['confirm' => 'Esta seguro de ENVIAR esta orden de insumos al modulo de inventario. Tener presente que este proceso es definitivo.', 'method' => 'post']]);?>
            <?php } 
        } ?> 
            ...
            <table class="table table-bordered table-striped table-hover">
                ...
                <tr style="font-size: 85%;">
                   <th style='background-color:#F0F3EF;'><?= Html::activeLabel($model, 'numero_orden') ?>:</th>
                    ...
                    <td ><?= Html::encode(''.number_format($model->total_insumos,0)) ?></td>
    ...
    <div>
        ...
        <div class="tab-content">
                ...
                <div class="table-responsive">
                    <div class="panel panel-success">
                                    ...
                                    $configuracion_insumos = app\models\ConfiguracionInsumos::findOne(1);
                                     foreach ($tallas as $val):?>
                                        <tr style='font-size:85%;'>
                                            ...
                                            <td><?= $val->cantidad ?></td>
                                            <?php if($model->autorizado == 0 && $configuracion_insumos->aplica_insumos_referencia == 1){?> 
                                            ...
                                            <?php } else {  //SE GENERAN INSUMOS GENERALES
                                               if($model->autorizado == 0 && $configuracion_insumos->aplica_insumos_ordenes == 1){?>
                                                   <td style= 'width: 20px; height: 20px' >
                                                       <a href="<?= Url::toRoute(["orden-produccion-insumos/ver_listado_insumos", "id" => $model->id_entrega]) ?>" ><span class="glyphicon glyphicon-eye-open"></span></a>
                                                    </td>    
                                               <?php }else{?>                              
                        ...
                        if($model->autorizado == 0){
                            echo Html::a('<span class="glyphicon glyphicon-plus"></span> Crear insumos', ['orden-produccion-insumos/crear_insumos_tallas', 'id' => $model->id_entrega,'id_orden' => $model->idordenproduccion], ['class' => 'btn btn-success btn-xs']);
                        }?>
                   </div>     
                    </div>   
                </div>
            ...
            <?php if($configuracion_insumos->aplica_insumos_referencia == 1){?>
                <!--INICIO EL OTRO TABS -->
                    ...
                    <div role="tabpanel" class="tab-pane " id="listadoinsumos">
                        <div class="table-responsive">
                            <div class="panel panel-success">
                                    ...
                                    <table class="table table-bordered table-hover">
                                        <thead>
                                            <tr style='font-size:85%;'>
                                              ...
                                              <th scope="col" style='background-color:#B9D5CE;'>Talla</th> 
                                               <th scope="col" style='background-color:#B9D5CE;'>Valor unidad</th> 
                                              <th scope="col" style='background-color:#B9D5CE;'>Unidades</th> 
                                              <th scope="col" style='background-color:#B9D5CE;'>Cantidad de metros</th> 
                                               <th scope="col" style='background-color:#B9D5CE;'>Total costo insumo</th>
                                              <th scope="col" style='background-color:#B9D5CE;'></th> 
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($listado as $val){?>
                                                <tr style='font-size:85%;'>
                                                    ...
                                                    <td style="text-align: right;"><?= ''. number_format($val->ordenDetalle->cantidad, 0)?></td>
                                                    <td style="text-align: right;"><?= ''. number_format($val->cantidad_despachada, 0)?></td>
                                                    <td><?= $val->ordenDetalle->listadoTalla?></td>
                                                    <td style="text-align: right;"><?= '$'. number_format($val->valor_unidad,0) ?></td>
                                                    <?php if($val->insumos->validar_item == 1){
                                                        if($val->insumos->tipomedida->abreviatura == 'Un'){ ?>
                                                            <td style="text-align: right; background-color: #B9D5CE">
                                                                <input type="text" name="unidades[<?= $val->id ?>]" style="text-align: right" size = "5" value="<?= $val->unidades ?>" >
                                                            </td>
                                                             <td></td>
                                                        <?php }else{?> 
                                                            <td style='text-align:right'>
                                                            <td style="text-align: right; background-color: #B9D5EE">
                                                                <input type="text" name="metros[<?= $val->id ?>]" style="text-align: right" size = "5" value="<?= $val->metros ?>" >
                                                            </td>
                                                        <?php }
                                                    }else{?>
                                                        <td></td>
                                                    ...
                                                    <?php } ?>  
                                                    <td style='text-align:right'><?= ''. number_format($val->total_insumo,0)?></td>
                                                    <?php if($model->autorizado == 0) {?>
                                                        <td style="width: 20px; height: 20px">	
                                                                <?= Html::a('', ['eliminar_insumos', 'id' => $model->id_entrega, 'id_detalle' => $val->id], [
                                ...
                                <div class="panel-footer text-right"> 
                                    <?php 
                                    if($model->autorizado == 0 && count($listado) > 0){
